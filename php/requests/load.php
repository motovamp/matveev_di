...
$stm = $dbh->prepare('UPDATE requests SET client_id=?, service_id=?, rq_date=?, rq_status=?, rq_pay=?, obj_sum=?, obj_rooms=?, obj_size=?, obj_floor=?, obj_floors=?, obj_district=?, obj_address=?, rq_note=? WHERE request_id=?');
$stm->execute(array($client_id, $service_id,/* $contract_num,*/ $rq_date, $rq_status, $rq_pay, $obj_sum, $obj_rooms, $obj_size, $obj_floor, $obj_floors, $obj_district, $obj_address, $rq_note, $request_id));
}
